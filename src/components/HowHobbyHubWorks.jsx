...
const HowHobbyHubWorks = () => {
  ...
  return (
    <section>
      <div className="bg-[#fcfcfd] py-8 md:py-16 border-y border-gray-100">
        <div className="max-w-7xl mx-auto px-6 md:px-12">

          {/* Header: Left Aligned for a more "Executive" look */}
          <div className="flex flex-col lg:flex-row lg:items-end justify-between mb-8 md:mb-16 gap-6">
            <div className="max-w-xl">
              ...
              <h2 className="text-4xl md:text-5xl font-tight tracking-tight text-slate-900 font-bold">
          ...
          {/* Steps: Clean Horizontal Grid with hairline dividers */}
          <div className="grid grid-cols-1 md:grid-cols-3 border-t border-gray-200">
            {steps.map((step, index) => (
              ...
              >
                {/* Step Tag */}
                <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-8 block group-hover:text-indigo-600 transition-colors">
                  {step.tag}
                </span>

                {/* Icon: Using simple shapes & high contrast */}
                <div className="w-12 h-12 mb-8 flex items-center justify-center bg-white shadow-sm border border-gray-100 rounded-lg text-2xl group-hover:shadow-md group-hover:border-indigo-100 transition-all">
                  {step.icon}
                </div>

                {/* Text: Precise and descriptive */}
                <h3 className="text-xl font-bold text-slate-900 mb-4 tracking-tight">
                  {step.title}
                </h3>
                <p className="text-slate-600 leading-relaxed text-sm lg:text-base antialiased">
                  {step.desc}
                </p>

                {/* Subtle Progress Bar */}
                <div className="mt-8 h-[2px] w-8 bg-gray-200 group-hover:w-full group-hover:bg-primary transition-all duration-700" />
              </div>
            ))}
          </div>
        </div>
